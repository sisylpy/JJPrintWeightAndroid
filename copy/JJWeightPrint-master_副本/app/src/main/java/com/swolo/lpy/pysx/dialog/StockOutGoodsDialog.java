    ...
    private BluetoothGattCallback gattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN_MR2) {
                Log.d("StockOutGoodsDialog", "[蓝牙] onConnectionStateChange: status=" + status + ", newState=" + newState);
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    Log.d("StockOutGoodsDialog", "[蓝牙] 连接成功，开始发现服务");
                    // 检查蓝牙连接权限
                    if (ContextCompat.checkSelfPermission(getContext(), Manifest.permission.BLUETOOTH_CONNECT) 
                            == PackageManager.PERMISSION_GRANTED) {
                        gatt.discoverServices();
                    } else {
                        Log.e("StockOutGoodsDialog", "[蓝牙] 缺少BLUETOOTH_CONNECT权限，无法发现服务");
                    }
                } else if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                    Log.d("StockOutGoodsDialog", "[蓝牙] 连接断开");
                }
            }
        }
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN_MR2) {
                Log.d("StockOutGoodsDialog", "[蓝牙] onServicesDiscovered: status=" + status);
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    Log.d("StockOutGoodsDialog", "[蓝牙] 服务发现成功");
                    // 查找特征
                    BluetoothGattService service = gatt.getService(UUID.fromString("0000ffe0-0000-1000-8000-00805f9b34fb"));
                    if (service != null) {
                        Log.d("StockOutGoodsDialog", "[蓝牙] 找到服务");
                        ...
                        if (writeCharacteristic != null) {
                            Log.d("StockOutGoodsDialog", "[蓝牙] 找到写特征");
                        }
                        if (notifyCharacteristic != null) {
                            Log.d("StockOutGoodsDialog", "[蓝牙] 找到通知特征");
                            // 检查蓝牙连接权限
                            if (ContextCompat.checkSelfPermission(getContext(), Manifest.permission.BLUETOOTH_CONNECT) 
                                    == PackageManager.PERMISSION_GRANTED) {
                                gatt.setCharacteristicNotification(notifyCharacteristic, true);
                            } else {
                                Log.e("StockOutGoodsDialog", "[蓝牙] 缺少BLUETOOTH_CONNECT权限，无法设置特征通知");
                            }
                        }
                    } else {
                        Log.e("StockOutGoodsDialog", "[蓝牙] 未找到服务");
                    }
                } else {
                    Log.e("StockOutGoodsDialog", "[蓝牙] 服务发现失败: " + status);
                }
            }
        }
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN_MR2) {
                Log.d("StockOutGoodsDialog", "[蓝牙] onCharacteristicChanged");
                byte[] data = characteristic.getValue();
                if (data != null && data.length > 0) {
                    Log.d("StockOutGoodsDialog", "[蓝牙] 收到数据: " + bytesToHex(data));
                    // 解析重量数据
...
protected void onCreate(Bundle savedInstanceState) {
    ...
    private void connectToScale() {
        ...
        if (scaleAddress != null) {
            ...
            if (ContextCompat.checkSelfPermission(getContext(), Manifest.permission.BLUETOOTH_CONNECT) 
                    != PackageManager.PERMISSION_GRANTED) {
                Log.e(TAG, "[蓝牙] 缺少BLUETOOTH_CONNECT权限，无法连接");
                return;
            }
            ...
            if (ContextCompat.checkSelfPermission(getContext(), Manifest.permission.BLUETOOTH) 
                    != PackageManager.PERMISSION_GRANTED) {
                Log.e(TAG, "[蓝牙] 缺少BLUETOOTH权限，无法连接");
                return;
            }
    ...
    private void parseWeightData(byte[] data) {
        if (data.length == 7 && data[0] == 0x05) {
            ...
            if (!autoTareDone && isStable) {
                ...
                autoTareDone = true;
                Log.d("StockOutGoodsDialog", "[蓝牙] 自动去皮完成，皮重: " + tareWeight + "g");
            }
            
            final double realWeight = Math.round((rawWeight - tareWeight) * 10) / 10.0;
            Log.d("StockOutGoodsDialog", "[蓝牙] 收到重量: " + realWeight + "g, 是否稳定: " + isStable);
            
            if (isStable) {
                Log.d("StockOutGoodsDialog", "[蓝牙] 重量稳定，准备写入输入框");
                setWeight(realWeight);
            }
        } else {
            Log.d("StockOutGoodsDialog", "[蓝牙] 数据格式不正确: 长度=" + data.length);
        }
    }
}